...
- (id)initWithTitle:(NSString *)aTitle options:(NSArray *)aOptions xy:(CGPoint)point size:(CGSize)size isMultiple:(BOOL)isMultiple parseKey:(NSString *)parseKey
{
    tempKey = parseKey;
    ...
    if (self = [super initWithFrame:rect])
    {
        ...
        if (isMultipleSelection) {
            ...
            [btnDone setFrame:CGRectMake(rect.size.width-90,rect.origin.y-45, 82, 31)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([self.arryData containsObject:indexPath]){
        imgarrow.frame=CGRectMake(((self.window.frame.size.width/2) - (287/2))-50,2, 27, 27);
        imgarrow.image=[UIImage imageNamed:@"check_mark@2x.png"];
	} else
        imgarrow.image=nil;
    
    [cell addSubview:imgarrow];
    cell.textLabel.text = [[_kDropDownOption objectAtIndex:row]valueForKey:tempKey] ;
    return cell;
}
...
-(void)SetBackGroundDropDown1_R:(CGFloat)r G:(CGFloat)g B:(CGFloat)b alpha:(CGFloat)alph {
    R=r;
    G=g;
    B=b;
    A=1;
